...
2. **`useCallback` (Performance Optimization / Function Memoization)**
...
3. **`useEffect` (Side-Effects Management)**
...
4. **`useRef` (DOM Reference & Direct Manipulation)**
...
## 5. Topic Reference Summary

| Topic / Feature Covered | Implementation Details |
| :--- | :--- |
| **State Declaration** | Declaring reactive state variables (`length`, `numberAllowed`, `charAllowed`, `password`) via `useState` |
| **Memoization** | Function optimization and preventing recreation on render using `useCallback` |
| **Password Logic** | Generating dynamic strings based on length and character sets |
| **Lifecycle Execution** | Automatically running generator on dependency updates using `useEffect` |
| **DOM Reference** | Attaching `useRef` to `<input>` element for direct DOM access |
| **Clipboard Copy** | Copying selected password text using Navigator Clipboard API |

---

## 💡 Key Takeaways & Best Practices
- **`useCallback` vs `useEffect`:** `useCallback` memoizes function definitions for performance; `useEffect` executes code when dependencies change.
- **Random Index Bounds:** Always use `Math.floor(Math.random() * str.length)` for 0-based string index lookup.
- **User Experience (UX):** Calling `select()` on `passwordRef.current` provides clear visual feedback upon copying.
